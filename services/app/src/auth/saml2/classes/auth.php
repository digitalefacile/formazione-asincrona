...
class auth extends \auth_plugin_base {
    ...
    public function error_page($msg) {
        ...
        echo $OUTPUT->single_button($logouturl, 'Torna alla home', 'get');
        echo $OUTPUT->footer();
        exit(1);
    }
    /**
     * Shows an error page for a user that are not present on the system but the auth is succefull.
     */
    public function error_page_custom(){
        global $PAGE, $OUTPUT, $SESSION;
        // Clean up $SESSION->wantsurl that was set explicitly in {@see auth_saml2\login},
        // we don't go anywhere.
        unset($SESSION->wantsurl);
        ...
        $PAGE->set_heading(get_string('error', 'auth_saml2'));
        echo $OUTPUT->header();
        echo '<div style="text-align: center;margin:10%;padding: 33px;">';
        echo '<p style="color:#000000;font-size:22px;">Siamo spiacenti, al momento non risulta un account a te collegato. Contatta la tua segreteria didattica per maggiori informazioni o richiedere assistenza.</p>';
