...
$duplicate_upload = get_config('local_zendesk', 'duplicate_upload') ? true : false;
    ...
    switch ($attachment['error']) {
        case UPLOAD_ERR_OK:
            ...
            if ($upload_httpcode == 201) {
                ...
                if ($duplicate_upload) {
                    // Secondo upload con inline=true
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL, "https://$subdomain.zendesk.com/api/v2/uploads.json?filename=" . urlencode($file_name) . "&inline=true");
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($ch, CURLOPT_POST, true);
                    curl_setopt($ch, CURLOPT_POSTFIELDS, $file_data);
                    curl_setopt($ch, CURLOPT_HTTPHEADER, [
                    ...
                    $inline_upload_response = curl_exec($ch);
                    $inline_upload_httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                    curl_close($ch);

                    if ($inline_upload_httpcode == 201) {
                        $inline_upload_data = json_decode($inline_upload_response, true);
                        $inline_upload_url = $inline_upload_data['upload']['attachment']['content_url'];

                        echo json_encode([
                            'success' => true,
                            'response' => 'Allegato caricato con successo',
                            'upload_token' => $upload_token,
                            'upload_url' => $inline_upload_url,
                            'duplicate_upload' => true
                        ]);
                        exit;
                    } else {
                        echo json_encode(['success' => false, 'response' => $inline_upload_response]);
                        exit;
                    }
                } else {
                    echo json_encode([
