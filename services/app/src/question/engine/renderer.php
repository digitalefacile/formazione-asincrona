...
require_once($CFG->dirroot . '/theme/edumy/classes/output/mod_quiz_renderer.php');
use theme_edumy\output\mod_quiz_renderer;
...
class core_question_renderer extends plugin_renderer_base {
    ...
    public function question(question_attempt $qa, qbehaviour_renderer $behaviouroutput,
        ...
        $quizattempt = quiz_attempt::create_from_usage_id($qa->get_usage_id()); # ADDED
        ...
        $output .= html_writer::start_tag('div', array('class' => 'content flex-grow-1 m-0 px-4'));
        ...
        $output .= $quizoutput->countdown_timer($quizattempt, time()); # ADDED
        $output .= html_writer::end_tag('div');
    ...
    protected function info(question_attempt $qa, qbehaviour_renderer $behaviouroutput,
        ...
        $quizattempt = quiz_attempt::create_from_usage_id($qa->get_usage_id()); # ADDED
        $totNumber = count($quizattempt->get_slots()); # ADDED
        
        $output = '';
        $output .= $this->number($number,$totNumber);
    ...
    protected function number($number,$totNumber) {
        ...
        if (trim($number) === 'i') {
            ...
            $numbertext = get_string('questionx', 'question',
                    html_writer::tag('span', s($number.' di '.$totNumber), array('class' => 'qno')));
        }
