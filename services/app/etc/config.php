...
$CFG->dbtype    = getenv('MOODLE_DATABASE_TYPE') ?: 'mysqli';     // 'pgsql', 'mariadb', 'mysqli', 'auroramysql', 'sqlsrv' or 'oci'
$CFG->dblibrary = 'native';     // 'native' only at the moment
$CFG->dbhost    = getenv('MOODLE_DATABASE_HOST') ?: 'localhost';  // eg 'localhost' or 'db.isp.com' or IP
$CFG->dbname    = getenv('MOODLE_DATABASE_NAME') ?: 'moodle';     // database name, eg moodle
$CFG->dbuser    = getenv('MOODLE_DATABASE_USER') ?: 'username';   // your database username
$CFG->dbpass    = getenv('MOODLE_DATABASE_PASSWORD') ?: 'password';   // your database password
$CFG->prefix    = getenv('MOODLE_DATABASE_PREFIX') ?: 'mdl_';       // prefix to use for all table names
$CFG->dboptions = array(
    ...
    'dbport'    => getenv('MOODLE_DATABASE_PORT_NUMBER') ?: '',          // the TCP port number to use when connecting
    ...
    // TODO: check if 'dbcollation' value in use right now ('utf8mb4_0900_ai_ci') has some issues
...
if(getenv('MOODLE_DATABASE_HOST_RO')) { // Set DB Reader Instance if provided
    $CFG->dboptions['instance'] = getenv('MOODLE_DATABASE_HOST_RO');
}
...
// $CFG->wwwroot   = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') ? 'https://' : 'http://') . getenv('MOODLE_HOST') ?: '127.0.0.1';
$CFG->wwwroot   = getenv('MOODLE_HOST') ?: 'http://127.0.0.1:8080';
...
// it may be also necessary to enable this when using port forwarding.
      $CFG->reverseproxy = getenv('MOODLE_REVERSEPROXY') == "true" ? true : false;
//
// Enable when using external SSL appliance for performance reasons.
// Please note that site may be accessible via http: or https:, but not both!
      $CFG->sslproxy = getenv('MOODLE_SSLPROXY') == "true" ? true : false;
//
...
//
     $CFG->tempdir = $CFG->dataroot . '/temp';        // Directory MUST BE SHARED by all cluster nodes.
     $CFG->cachedir = $CFG->dataroot . '/cache';      // Directory MUST BE SHARED by all cluster nodes, locking required.
     $CFG->localcachedir = '/var/www/localmoodledata/localcache';      // Intended for local node caching.
     $CFG->localrequestdir = '/tmp';                  // Intended for local only temporary files. The defaults uses sys_get_temp_dir().
     $CFG->alternative_component_cache = '/var/www/localmoodledata/cache/core_component.php';
//
...
//
     $CFG->backuptempdir = $CFG->dataroot . '/backuptemp';  // Directory MUST BE SHARED by all cluster nodes.
//
